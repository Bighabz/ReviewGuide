...
class ClarifierAgent(BaseAgent):
    ...
    async def _handle_new_plan(self, state: GraphState, session_id: str) -> Dict[str, Any]:
        ...
        # ask. USE CASE comes first ("What will you mainly use it for?") because it
        # determines everything downstream; BUDGET comes last, with category-realistic
        # price brackets generated by the LLM (see _generate_followup_questions) —
        # never a generic "Under $50" tier list. Both are OPTIONAL slots on
        # product_search, so the clarifier would never ask them on its own.
        if intent == "product":
            ...
            if substantive:
                if not has_use_case and "use_case" not in missing_required_slots:
                ...
                if not has_budget and "budget" not in missing_required_slots:
            ...
            # Ordering: use_case always first, budget always last.
...
For EVERY question, also generate "options": 3-5 short tappable answer choices that are REALISTIC for this specific product category:
- use_case options = the genuinely common uses for this category (laptops: "Student / everyday", "Gaming", "Creative / video editing", "Business / office"; bikes: "Road", "Mountain", "Commuting", "Casual rides")
- budget options = realistic price brackets for THIS category's actual market (laptops: "Under $500", "$500–$800", "$800–$1,200", "$1,200+"; mattresses: "Under $500", "$500–$1,000", "$1,000–$2,000", "$2,000+"). NEVER offer brackets that make no sense for the category (no "Under $50" for laptops).
- other slots = the 3-5 most common real answers for that category (actual brands, sizes, styles)
Each option is 1-4 words. Also generate "free_text_hint": a short affordance like "or describe your own use" / "or type an amount".
