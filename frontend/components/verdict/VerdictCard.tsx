 ...
 *   - --accent (terracotta) is reserved for money actions and verdict labels.
 *   - Rank is typography (serif numeral), never a badge chip.
 *   - Badges come from data (badges[] / best_price), never from array position.
 *   - Missing images get a designed monogram tile, not an emoji.
 *   - Tokens only — no raw hex, no Tailwind `dark:` variants.
 *   - Save bookmarks + affiliate click tracking are preserved from the
 *     blueprint ProductCarousel.
 */
...
export interface ProductInput {
  ...
  badges?: string[]
  // carousel-item fields (blueprint ProductCarousel shape)
  product_id?: string
  description?: string
  best_price?: boolean
  savings?: number
  compared_retailer?: string
  /** Offer-condition label from compose (#104): "Renewed", "Used", "Open box"… */
...
export function normalizeProduct(p: ProductInput, index = 0): NormalizedProduct {
  ...
  if (p.condition_label && !badges.includes(p.condition_label)) {
    badges.push(p.condition_label)
  }

  const price = p.price ?? p.best_offer?.price
  ...
  return {
    ...
    badges,
    savings: p.savings,
    comparedRetailer: p.compared_retailer,
  }
}
...
/** Designed missing-image fallback: serif monogram on terra-soft — an editorial drop cap, not an emoji */
...
/** Bookmark toggle — ported from the blueprint ProductCarousel so Saved/Compare keep working. */
function SaveToggle({ product }: { product: NormalizedProduct }) {
  const [saved, setSaved] = useState(false)
  const [pulse, setPulse] = useState(0)
  const id = slugifyProduct(product.title)
  useEffect(() => {
    setSaved(isSaved(id))
  ...
  const item: Omit<SavedItem, 'savedAt'> = {
    id,
    name: product.title,
    price: product.price,
    imageUrl: product.imageUrl,
    url: product.link,
    role: product.badges[0] || product.merchant,
  }
  return (
    <button
      onClick={(e) => {
    ...
    >
      {pulse > 0 && (
        <span key={pulse} className="rg-ring absolute inset-0 rounded-full" style={{ border: '1px solid var(--accent)' }} />
      )}
      <Bookmark
        key={`${saved}-${pulse}`}
        size={15}
        ...
        style={{ color: 'var(--accent)', fill: saved ? 'var(--accent)' : 'transparent' }}
      />
    </button>
  )
}
...
function ProductImage({
  ...
  return (
    <div className={`relative overflow-hidden ${className}`} style={{ background: 'var(--surface)' }}>
      ...
      {withSave && <SaveToggle product={product} />}
    </div>
  )
}
...
function BuyButton({ product, full = false }: { product: NormalizedProduct; full?: boolean }) {
  ...
  return (
    ...
    >
      {product.price != null ? 'See price' : 'Check price'}
      {product.merchant ? ` at ${product.merchant}` : ''}
...
function Badges({ badges, merchant }: { badges: string[]; merchant?: string }) {
  ...
  return (
    <p className="flex flex-wrap items-baseline gap-x-2.5 gap-y-1 text-[11px] font-semibold uppercase tracking-[0.16em]">
      ...
      {merchant && badges.length === 0 && <span style={{ color: 'var(--text-muted)' }}>via {merchant}</span>}
    </p>
  )
}
  ...
  return (
    ...
    >
      <div className="flex flex-col md:flex-row">
        <ProductImage product={product} withSave className="md:w-[42%] aspect-[4/3] md:aspect-auto md:min-h-[320px] shrink-0" />
...
function StandardCard({ product }: { product: NormalizedProduct }) {
  return (
    ...
    >
      <div className="flex">
        <ProductImage product={product} withSave className="w-28 sm:w-40 shrink-0 self-stretch min-h-[9rem]" />
...
function RailCard({ product }: { product: NormalizedProduct }) {
  ...
  return (
    <article
      ...
      style={{ background: 'var(--surface-elevated)', border: '1px solid var(--border)' }}
    >
      <ProductImage product={product} withSave className="aspect-[4/3]" />
      <div className="p-4 flex flex-col gap-2 flex-1">
        ...
        {product.savings != null && product.savings > 0 && (
          <p className="text-[11px] font-medium" style={{ color: 'var(--accent)' }}>
            ...
            {product.comparedRetailer ? ` vs ${product.comparedRetailer}` : ''}
          </p>
        )}
        <div className="mt-auto pt-3 flex items-center justify-between gap-2" style={{ borderTop: '1px solid var(--border)' }}>
          ...
          <a
            ...
            style={{ background: 'var(--accent)', color: 'var(--surface-elevated)' }}
          >
            {product.price != null ? 'Price' : 'Check price'} <ArrowUpRight size={12} className="ml-0.5" />
          </a>
        </div>
      </div>
    </article>
