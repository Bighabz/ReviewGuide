...
**Brand tagline:** *"Ask before you buy."* This is the product's positioning sentence and appears on the logo lockup throughout the app. It is static everywhere except the Discover hero, where the final word rotates (Buy, Eat, Fly, Stay, Book, Subscribe…) to signal the product's range across verticals.
...
**MVP status:** **Static placeholder only.** The full personality-memory infrastructure (Honcho-backed persistent profile, learned attributes, register inference) is deferred to post-MVP. For v1, this screen exists in the IA and renders the empty-state copy *"Still getting to know you. Ask a few things and this fills in."* — but the AI does not yet write to it. The placeholder establishes the trust surface visually and reserves the route. The full spec below describes the post-MVP target.
...
The ReviewGuide wordmark is the existing **"ReviewGuide.Ai"** lockup, rebuilt in the new color system:

- **"Review"** and **".Ai"** in **Terracotta `#B8543A`** (was saturated cyan)
- **"Guide"** in **Ink `#1A1816`** (was dark navy)
- Set on **Cream `#FAFAF7`**
- Speech-bubble container in the terracotta stroke when used in the Discover hero treatment; omitted on every other page

The wordmark has two treatments:

- **Discover hero (full bubble + rotating word)** — the lockup sits inside its speech-bubble outline, large, centered. Below the wordmark, the tagline reads *"Ask Before You [word]"* where the final word rotates approximately every 2.4 seconds through: Buy → Eat → Fly → Stay → Book → Subscribe. The serif greeting *"What are you researching?"* sits beneath the bubble, smaller, in Instrument Serif Italic. The bubble and the greeting are stacked, both visible.
- **Every other page (small static wordmark)** — the wordmark appears in the top-left of the page header, scaled down, **without the bubble outline**. The tagline below it reads the static *"Ask before you buy."* No rotation.

The bubble outline is the brand's only formal nod to the chatbot-app visual vocabulary. It exists once, on the entry screen. After that, the product is editorial.
...
- **Personality memory infrastructure (Honcho or equivalent).** The §7.10 personality profile screen exists in MVP as a static placeholder. The actual learned-attribute system — Honcho-backed persistent profile, register inference, priorities extraction — is deferred to post-MVP. The screen reserves the route and the visual surface; the AI does not yet read or write to it in v1.
